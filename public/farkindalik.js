(function () {
  ...
  const kidsGame = root.querySelector('[data-kids-game="bag"]');
  if (kidsGame) {
    ...
    const picked = new Map(); // key -> button
    const maxCorrect = 6;

    function setScore() {
      scoreEl.textContent = `${Math.min(picked.size, maxCorrect)} / ${maxCorrect}`;
    }
    ...
    btnCheck?.addEventListener("click", () => {
      ...
      for (const [, btn] of picked) {
        const isCorrect = btn.getAttribute("data-kids-correct") === "1";
        if (isCorrect) correct += 1;
        else wrong += 1;
      }

      if (picked.size === 0) {
        showFeedback("warn", "<strong>Hadi başlayalım.</strong><p>Çantaya en az 3 temel malzeme ekleyip tekrar kontrol et.</p>");
        return;
      }

      if (wrong === 0 && correct >= 4) {
        showFeedback(
          "ok",
          `<strong>Harika!</strong><p>${correct} doğru seçim yaptın. Su, fener, düdük ve ilk yardım gibi temel malzemeler çok önemli.</p>`
        );
        return;
      }
      ...
      showFeedback(
        "fail",
        `<strong>Biraz daha düşünelim.</strong><p>${correct} doğru, ${wrong} gereksiz seçim var. Gereksizleri çantadan çıkarıp tekrar dene.</p>`
      );
    });
